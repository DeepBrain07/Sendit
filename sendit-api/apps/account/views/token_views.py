...
from apps.account.documentation.account.schemas import (
    ...
    password_reset_verify, profile_doc, refresh_token_doc, get_me_doc
)
...
class LoginView(GenericAPIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            print(f"[token login] wallet {wallet.virtual_account_bank_number, wallet.id,wallet.virtual_account_number}")
...
class MeView(GenericAPIView):
    ...
    def get_queryset(self):
        ...
        Prefetch(
            'media',
            queryset=Media.objects.filter(tag='avatar'),
            to_attr='prefetched_avatar'
        )
    )
...
class ProfileView(GenericAPIView):
    ...
    def get_object(self):
        ...
        # Optional: check permission early
        if not (self.request.user.is_staff or self.request.user.id == user_id):
            raise Http404("Not allowed")

        # Build optimized queryset
        queryset = Profile.objects.filter(user__id=user_id)
        # .prefetch_related(
        #     Prefetch('media', queryset=Media.objects.filter(tag='avatar'))
        # )
